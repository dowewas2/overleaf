...
\begin{frame}{}
  \begin{columns}[t]
       ...
       %-- Column 1 ---------------------------------------------------
    \begin{column}{0.282\linewidth} % you can adjust width as needed but total must be 1 and should be close to 1/ each with center largest
...
\begin{figure}[h!] 
\centering
\scalebox{.60}{\includegraphics{genpetlabe.eps}}
...
\item[]{\bf Corollary} For any $n \geq 3$ and $0 <  k  \leq \lfloor \frac{n-1}{2} \rfloor$, such that there is $\lambda \in spec(A(C_{n,k}))$ such that the multiplicity of $\lambda$ is  $2k+2$, we have $\M(C_{n,k}) = \Z(C_{n,k}) = 2k+2$.\\*[1cm]
